...
import {ADD_DOCUMENTS} from "./mutation/addDocuments";
...
const ClaimRestaurant = () => {
    ...
    const onDrop = useCallback(async (acceptedFiles) => {
        ...
        for(let index = 0; index < postUrls.length; index++) {
            const fileSizeCheck = acceptedFiles[index].size <= 1000000;
            const fileTypeCheck = acceptedFiles[index].type === "application/pdf";

            if (!fileSizeCheck) {
                setMessage({
                    type: "failure",
                    message: "Total Files Size should be <= 10MB"
                });
                return;
            }
        ...
        if (response.data.addDocuments.code != 200) {
